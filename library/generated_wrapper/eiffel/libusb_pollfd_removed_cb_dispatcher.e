...
feature -- Initialization
	...
	make
		...
		do
			set_libusb_pollfd_removed_cb_object ($Current)
		end

feature -- Access: Routine 

	routine_1: detachable PROCEDURE [TUPLE [a_fd: INTEGER; a_user_data: POINTER]] 
			--Eiffel routine to be call on callback.

	routine_2: detachable PROCEDURE [TUPLE [a_fd: INTEGER; a_user_data: POINTER]] 
			--Eiffel routine to be call on callback.

	routine_3: detachable PROCEDURE [TUPLE [a_fd: INTEGER; a_user_data: POINTER]] 
			--Eiffel routine to be call on callback.
...
feature -- Access: Dispatcher
	...
	c_dispatcher_1: POINTER
		...
		do
			Result := get_libusb_pollfd_removed_cb_stub_1
		end
	...
	c_dispatcher_2: POINTER
		...
		do
			Result := get_libusb_pollfd_removed_cb_stub_2
		end
	...
	c_dispatcher_3: POINTER
		...
		do
			Result := get_libusb_pollfd_removed_cb_stub_3
		end

feature -- Access: Callback

	on_callback_1 (a_fd: INTEGER; a_user_data: POINTER)  
			-- Callback target.
		do
			if attached routine_1 as l_routine then 
				l_routine (a_fd, a_user_data)
			end
		end

	on_callback_2 (a_fd: INTEGER; a_user_data: POINTER)  
			-- Callback target.
		do
	...
	on_callback_3 (a_fd: INTEGER; a_user_data: POINTER)  
			-- Callback target.
		do
...
feature -- Register: Callbacks
	...
	register_callback_1 (a_routine: like routine_1)
		...
		do
			routine_1 := a_routine
			set_libusb_pollfd_removed_cb_entry_1 ($on_callback_1)
		ensure
	...
	register_callback_2 (a_routine: like routine_1)
		...
		do
			routine_2 := a_routine
			set_libusb_pollfd_removed_cb_entry_2 ($on_callback_2)
		ensure
	...
	register_callback_3 (a_routine: like routine_1)
		...
		do
			routine_3 := a_routine
			set_libusb_pollfd_removed_cb_entry_3 ($on_callback_3)
		ensure
...
feature {NONE} -- Implementation
	...
	dispose
		...
		do
			...
			set_libusb_pollfd_removed_cb_object (default_pointer)
		end
